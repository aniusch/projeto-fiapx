...
| Must not lose a request during spikes | Gateway only enqueues and returns **202**; durable RabbitMQ queue with persistent messages + DLQ ([`internal/messaging`](../internal/messaging)) | e2e: upload lands a message in `videos.jobs`; worker drains it. Load: `make spike` ([`load/spike.js`](../load/spike.js)) — k6 spike of ~960 uploads, asserts 0 rejected and 0 lost |
